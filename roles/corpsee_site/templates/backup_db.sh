...
function perform_backups()
{
    ...
    if [ ! -f "${BACKUP_DIR}/{{ corpsee_site_db_name }}_${CURRENT_DATE}_${SUFFIX}.sql.xz" ];
    then
        COMMAND="pg_dump -O --column-inserts -F p -U {{ corpsee_site_db_user }} -d {{ corpsee_site_db_name }} -h 127.0.0.1 | grep -v -E '^(CREATE\ EXTENSION|COMMENT\ ON)' | xz > ${BACKUP_DIR}/{{ corpsee_site_db_name }}_${CURRENT_DATE}_${SUFFIX}.sql.xz"
        eval "${COMMAND}"
    fi
}
